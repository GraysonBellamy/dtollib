...
from dtollib.errors import DtolSinkDependencyError, DtolSinkSchemaError, DtolSinkWriteError
from dtollib.sinks._schema import ColumnSpec, SchemaLock
from dtollib.sinks.base import block_to_rows, reading_to_row
...
def _load_pyarrow() -> tuple[Any, Any]:
    """Lazy-import pyarrow; raise :class:`DtolSinkDependencyError` on miss."""
    try:
        ...
        raise DtolSinkDependencyError(
            ...
            "Install with: `uv add 'dtollib[parquet]'` "
            "(or `pip install 'dtollib[parquet]'`)."
        ) from exc
    return pa, pq
...
class ParquetSink:
    ...
    def _write_rows(self, rows: list[dict[str, float | int | str | bool | None]]) -> None:
        ...
        try:
            ...
            raise DtolSinkWriteError(f"ParquetSink: write failed for {self._path}: {exc}") from exc
