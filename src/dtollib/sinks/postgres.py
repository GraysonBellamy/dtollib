...
from dtollib.sinks.base import reading_to_row
...
def _block_summary_row(block: DaqBlock) -> dict[str, float | int | str | bool | None]:
    ...
    Mirrors :func:`dtollib.sinks.sqlite._block_summary_row`. The block's
    ``data`` array is intentionally not serialised — use
    :class:`~dtollib.sinks.parquet.ParquetSink` for sample-level archives.
    """
    return {
        "device": block.device,
        "task": block.task,
...
def _load_asyncpg() -> Any:
    """Lazy-import asyncpg; raise :class:`DtolSinkDependencyError` on miss."""
    try:
        import asyncpg  # pyright: ignore[reportMissingImports, reportMissingTypeStubs]  # noqa: PLC0415
    except ImportError as exc:
        raise DtolSinkDependencyError(
            "PostgresSink requires the `postgres` extra. "
            "Install with: `pip install 'dtollib[postgres]'` "
            "(or `uv add 'dtollib[postgres]'`).",
        ) from exc
    return asyncpg
