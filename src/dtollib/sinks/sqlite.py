...
def _block_summary_row(block: DaqBlock) -> dict[str, float | int | str | bool | None]:
    ...
    fields plus the raw channel list and units. The ``data`` array is
    intentionally **not** serialised; consumers who need samples should use
    :class:`~dtollib.sinks.parquet.ParquetSink` or scalarise explicitly via
...
class SqliteSink:
    ...
    def _executemany_blocking(
        ...
        try:
            ...
            raise DtolSinkWriteError(f"SqliteSink: INSERT failed: {exc}") from exc
