...
Row helpers convert acquisition records into row dicts for the tabular
sinks (SQLite / Parquet / Postgres):

- :func:`reading_to_row` — flatten :class:`DaqReading` into one row.
- :func:`block_to_rows` — explicit per-(channel, sample) scalarisation of a
  :class:`DaqBlock` into row dicts. Distinct from
  :func:`~dtollib.tasks.models.block_to_long_rows`, which yields typed
  :class:`~dtollib.tasks.models.DaqSample` objects; this one yields plain
  dicts ready for a tabular sink. Never invoked automatically.
"""
